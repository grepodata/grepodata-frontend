...
export class ScoreboardComponent implements OnInit {
  ...
  constructor(private globals: Globals,
    ...
    this.route.queryParams.subscribe( params => {
      if (params.world != undefined && params.date != undefined) this.router.navigate(['/points/'+params.world+'/'+params.date]);
  ...
  setInputValue (val) {
	...
	filterEvent() {
		...
		if (this.playerInput.length > 2) {
			this.searching = true;
			this.searchService.searchPlayers(this.playerInput, 0, 7, this.server, this.world, false, null, null, true, '')
				.subscribe(
  ...
  mapNav(event, modX, modY) {
  	...
  	this.zoomOriginY += modY * mod;
		this.renderer.setStyle(this.worldMapContainer.nativeElement, 'transform-origin', this.zoomOriginX+'px '+this.zoomOriginY+'px');
		this.buildCanvas();
	}
		...
		if (this.mapCanvas != null && !this.mobile && !this.animated) {
			...
			Object.keys(this.legend).forEach(k => {
				if (k === colorId) {
					...
					let colorDistance = Math.abs(pixelData[0] - this.legend[k].pixel[0]) +
			...
			if (match != null) {
				...
				// Create tooltip
				this.renderer.setStyle(this.mapTip.nativeElement, 'background', 'url('+this.worldMap.nativeElement.src+') -'+ legendData.namex +'px -'+ legendData.namey +'px no-repeat');
  ...
  load(params) {
    ...
    this.setInputValue('');
    this.scoreboardService.loadPlayerScoreboard(this.world, params['date'], this.server)
      .subscribe(
    ...
    this.scoreboardService.loadAllianceScoreboard(this.world, params['date'], this.server)
      .subscribe(
        (response) => this.renderAllianceScoreboard(response, params['date']),
        (error) => this.renderAllianceScoreboard(null, params['date'])
      );

    this.scoreboardService.loadAllianceChanges(this.world, params['date'], this.server, 0, 22)
      .subscribe(
  ...
  setDate(event) {
    let dateString = event.getFullYear() + '-' + ('0' + (event.getMonth()+1)).slice(-2) + "-" + ('0' + event.getDate()).slice(-2);
  ...
  prevDay() {
    ...
    this.router.navigate(['/points/'+this.world+'/'+((tomorrow.getFullYear().toString()) + '-' + ('0' + (tomorrow.getMonth()+1)).slice(-2) + "-" + ('0' + tomorrow.getDate()).slice(-2))]);
  }
  nextDay() {
    let today = new Date(this.selectedDate);
    let tomorrow = new Date(this.selectedDate);
  ...
  renderPlayerScoreboard(json, date) {
    if (json == null) {
      ...
      // Check response date
      if (date != undefined && date != json.date) this.noticePlayer = 'Unable to find player scoreboard for ' + this.world + ' on ' + date + '; showing today\'s scoreboard instead.';
  	...
  	console.log("Reloading map");
		setTimeout(_ => {
			if (this.showMap && this.worldMap && !this.mobile) {
				...
				let url = this.env.url + '/m/' + this.world +'/map_' + this.selectedDate.toString().replace(/-/g, '_') + '.png';
				if (this.env.production) {
  ...
  buildCanvas() {
  	if (!this.mobile && !this.animated) {
  		// Check initial origin
			...
			canvas.getContext('2d').drawImage(this.worldMap.nativeElement, 0, 0,
				this.worldMap.nativeElement.width, this.worldMap.nativeElement.height);
  ...
  loadPlayerDiffs() {
    this.scoreboardService.loadPlayerDiffs(this.world)
      .subscribe(
        (response) => {
  ...
  renderAllianceScoreboard(json, date) {
    if (json == null) {
      ...
      if (date != undefined && date != json.date) this.noticeAlliance = 'Unable to find alliance scoreboard for ' + this.world + ' on ' + date + '; showing ' + json.date + ' instead.';
...
export class OverviewDialog implements AfterViewInit {
  ...
  constructor(
    ...
    this.loading = true;
    this.scoreboardService.loadHourDiffs(this.world, this.date, this.hour)
      .subscribe(
...
export class AllianceOverviewDialog implements AfterViewInit {
  ...
  constructor(
    ...
    this.loading = true;
    this.scoreboardService.loadAllianceDayDiffs(this.world, this.date, this.alliance_id)
      .subscribe(
        (response) => this.renderResults(response),
        (error) => {console.log(error); this.error = true; this.loading = false;}
      );
