...
export class AllianceComponent implements AfterViewInit {
  ...
  private load(params) {
    ...
    this.allianceService.loadAllianceInfo(params['world'], params['id'])
      .subscribe(
        ...
        () => {
          // Load history
          this.allianceService.loadAllianceHistory(params['world'], params['id'])
            .subscribe(
              (response) => {
	...
	renderPlayerActivity($event) {
		if (!this.loadingMembers && hasOwnProperty(this.allianceMembersData, 'members')) {
			...
			this.allianceMembersData.members.forEach(i => {
				if (i.heatmap && i.heatmap.hour && hasOwnProperty(i.heatmap.hour, ''+this.selectedHour)) {
					activityNowHour.push({id: i.id, name: i.name, rank: i.rank, value: 1+i.heatmap.hour[this.selectedHour]})
				} else {
					activityNowHour.push({id: i.id, name: i.name, rank: i.rank, value: 1, label: 0})
				}
				if (i.heatmap && i.heatmap.day && hasOwnProperty(i.heatmap.day, ''+this.selectedDay)) {
					activityNowDay.push({id: i.id, name: i.name, rank: i.rank, value: 1+i.heatmap.day[this.selectedDay]})
				} else {
					activityNowDay.push({id: i.id, name: i.name, rank: i.rank, value: 1, label: 0})
				}
			});
			activityNowHour.sort((a, b) => (a.value > b.value) ? 1 : (a.value === b.value) ? ((a.rank > b.rank) ? 1 : -1) : -1);
			activityNowDay.sort((a, b) => (a.value > b.value) ? 1 : (a.value === b.value) ? ((a.rank > b.rank) ? 1 : -1) : -1);
  ...
  private loadMembers() {
    this.allianceService.loadAllianceMembers(this.world, this.id)
      .subscribe(
        (response) => {
  ...
  private loadChanges() {
    this.allianceService.loadAllianceChanges(this.world, this.id)
      .subscribe(
